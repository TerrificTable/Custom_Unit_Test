...
public class ExpectSmallerOrEqual extends Assertions {

    public <T> ExpectSmallerOrEqual(T expected, T actual) {
        total++;
        ...
        if (!((int) expected <= (int) actual)) {
            System.out.println("\033[91m`" + caller_class + "/" + caller_method + "/" + this.getClass().getSimpleName() + "` Failed:\n \tExpected smaller than or equal to: \n\t\t" + expected + " \n\tGot: \n\t\t" + actual + "\n\033[0m");
            assertions_failed.add("Failure in `" + caller_class + "/" + caller_method + "/" + this.getClass().getSimpleName() + "`: Expected smaller than or equal to: " + expected + " \tGot: " + actual);
            fails++;
        } else {
